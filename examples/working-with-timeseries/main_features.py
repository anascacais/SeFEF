...
def main(data_folder_path=data_folder_path, patient_id=patient_id, sampling_frequency=sampling_frequency):
    ...
    with h5py.File(os.path.join(preprocessed_data_path, f'dataset.h5'), 'r+') as h5dataset:
        if 'annotations' not in h5dataset.keys():
            labeling.add_annotations(
                h5dataset, sz_onsets_ts=dataset.metadata[dataset.metadata['sz_onset'] == 1].index.to_numpy())
    ...
    with h5py.File(os.path.join(preprocessed_data_path, f'dataset.h5'), 'r') as h5dataset:
        ...
        for ifold, (train_data, test_data) in enumerate(tscv.iterate(h5dataset)):
            ...
            scorer = scoring.Scorer(metrics2compute=metrics2compute, sz_onsets=sz_onsets_test, forecast_horizon=60 *
                                    60, reference_method='prior_prob', hist_prior_prob=len(sz_onsets_train)/len(y_train))
            fold_performance = scorer.compute_metrics(
                forecasts, ts, binning_method='equal_width', num_bins=10, draw_diagram=False)
